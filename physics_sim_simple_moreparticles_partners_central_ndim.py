...
# n dimensions
def random_n_vector_with_magnitude(magnitude, dimensions=partdim):
...
class Particle():
    ...
    def __init__(self, position, velocity, mass, friction , force , temperature, particle_type = None, interaction_partners = [], interaction_group = 0, group=0, entity_group=0, move_group=0, full_group=0, number=0, central_particle=False, central_particle_group=0, identity=0,
                ...
                active_velocity0 = 1, active_velocity_vec=None, sab=False, eab=False):
        ...
        if self.particle_type == "const":
            self.const_vel = random_n_vector_with_magnitude(self.active_velocity0, partdim)
...
def add_noise_with_drift(vector, noise_magnitude=0, drift_vector=None, drift_magnitude=0.0):
    vector = np.asarray(vector)
    
    noise = np.random.randn(*vector.shape)
    noise /= np.linalg.norm(noise)  # Normalize to unit vector
    noise *= noise_magnitude        # Scale to desired noise magnitude

    # Generate drift
    
    if drift_vector is not None:
        drift_vector_norm = np.linalg.norm(drift_vector)
        if drift_vector_norm != 0:
            drift_vector = np.asarray(drift_vector)
            drift = (drift_vector / np.linalg.norm(drift_vector)) * drift_magnitude
        else:
            drift = np.zeros_like(vector)  # No drift
    else:
        drift = np.zeros_like(vector)  # No drift

    return vector + noise + drift

def update_active_velocities(active_vels):
    for ia, active_vel in enumerate(active_vels):

        active_vels[ia] = add_noise_with_drift(active_vel, noise_magnitude=random_noise, drift_vector=drift_vector, drift_magnitude=drift_magnitude)
...
def new_vel_partners_global_velocities(Particle):
    ...
    if Particle.particle_type == "const":
        if const_vel_try == True:
            #Particle.velocity = Particle.const_vel
            Particle.velocity = Particle.active_velocity_vec
            Particle.force = np.array([0 for _ in range(partdim)])
...
def beads_distance_simple(formerpos, desireddist):
    formerpos = np.asarray(formerpos)
    n = formerpos.shape[0]
    ...
    newpoint = formerpos + direction * desireddist
    return newpoint



def random_vector_with_length(length, n=partdim):
                ...
                for iip, particle_entry in enumerate(particle_group):    
                    ...
                    if not curr_central_particle:
                        ...
                        if cpart_type == "sab":
                            ...
                            particles.append(Particle(nextpos,initvel,1,initfriction,initforce,entity_temps[ip], active_velocity_vec=init_active_velocity_vec,active_velocity0=active_velocity0 ,particle_type="const", sab=True, identity = particle_entry, interaction_partners=curr_group, interaction_group=ip, central_particle = curr_central_particle, central_particle_group= central_particles[ip], group=ip, entity_group=ip, move_group=ip, number=particle_entry))    
                        else:
...
def init_particles_partners_simple_central_allsab_global_vel(temp = [0.1,0.1,0.2,0.2], entities=2, cpart_type="const", 
                                               active_velocity0=1, move_groups_nr=5, furtherparticles=50, free_particles=None, 
                                               random_distr_of_move_groups=False, desireddist = 0.8, friction = 1, explicit_groups=None):
    import math 
    ...
    for ip, particle_group in enumerate(list_of_lists_fixed):
        ...
        nextpos = initpos
        for iip, particle_entry in enumerate(particle_group):    
            nextpos = beads_distance_simple(nextpos, desireddist)
            '''
            if partdim == 3:
                nextpos = beads_distance_3d(nextpos, desireddist, particles)
            elif partdim == 2:
...
def particles_init_simulate_smallgroups_sab_velocity_objects(stim_steps=1000, Dvar=0.1, update_global_activity_vector_var = True, change_groups_after_steps_var=100, invert_velocity_after_steps_var=50, move_groups_nr=5, furtherparticles=50, T=1, dt = 0.01, act_magnitude=1, box_variable = False, boxl_variable=50, replace_vel_variable=True, adopt_vel_var=True, adopt_act_val_var=False, partdim_variable=3, cpart_type="const", only_central_interactions_variable = True, const_vel_try_variable = False, interaction_partner_calculation = True,  group_thermostat=False, harmpot=1, thermostat=1, fenepot=0, random_noise_act_var=0, drift_vector_var=None, drift_magnitude_var=1):
    ...
    for group_vel in range(move_groups_nr):
        active_velocity_vec = random_n_vector_with_magnitude(magnitude=1, dimensions=partdim)
    ...
    global random_noise
    global drift_vector
    global drift_magnitude
    random_noise = random_noise_act_var
    drift_vector = drift_vector_var
    drift_magnitude = drift_magnitude_var


    particles = init_particles_partners_simple_central_allsab_global_vel(cpart_type="const", temp=T, active_velocity0=act_magnitude, move_groups_nr=move_groups_nr, furtherparticles=furtherparticles)
